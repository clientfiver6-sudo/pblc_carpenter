...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  let query = db.from("prescriptions").select("*").eq("business_id", businessId).order("created_at", { ascending: false })
  if (customerId) query = query.eq("customer_id", customerId)

  const { data, error } = await query
  if (error) return NextResponse.json({ error: (error as { message: string }).message }, { status: 500 })
  return NextResponse.json(data as Prescription[])
}
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  ...
  const { error } = await db.from("prescriptions").delete().eq("id", id).eq("business_id", businessId)
  if (error) return NextResponse.json({ error: (error as { message: string }).message }, { status: 500 })
